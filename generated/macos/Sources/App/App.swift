...
struct OwlNativeGeneratedApp: App {
  ...
  var body: some Scene {
    ...
    .commands {
      CommandMenu("Owl") {
        Button("Refresh") { session.perform(action: "refresh_snapshot") }
          .keyboardShortcut("r", modifiers: [.command])
        Button("Check SimpleX") { session.perform(action: "tick_simplex") }
        Divider()
        Button("Settings") { session.perform(action: "open_settings") }
          .keyboardShortcut(",", modifiers: [.command])
      }
      CommandMenu("Mailbox") {
        Button("Inbox") { session.perform(action: "focus_inbox") }
          .keyboardShortcut("1", modifiers: [.command])
        Button("Favorites") { session.perform(action: "focus_favorites") }
          .keyboardShortcut("2", modifiers: [.command])
        Button("People") { session.perform(action: "focus_people") }
          .keyboardShortcut("3", modifiers: [.command])
        Button("Groups") { session.perform(action: "focus_groups") }
          .keyboardShortcut("4", modifiers: [.command])
      }
      CommandMenu("Transport") {
        Button("Use SimpleX") { session.perform(action: "compose_simplex") }
        Button("Use Email") { session.perform(action: "compose_email") }
      }
    }
    Settings {
      SettingsView()
