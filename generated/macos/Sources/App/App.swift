...
private struct CardStackFrame<Content: View>: View {
  ...
  private var cardFill: LinearGradient {
    LinearGradient(
      colors: [
        tint.opacity(isSelected ? 0.24 : 0.16),
        Color(nsColor: .controlBackgroundColor).opacity(0.96),
        tint.opacity(isSelected ? 0.12 : 0.08)
      ],
  ...
  var body: some View {
    content
        ...
        ZStack {
          ForEach(0..<backCount, id: \.self) { index in
            RoundedRectangle(cornerRadius: 8)
              ...
              .offset(x: stackOffsetX(index), y: -CGFloat(index + 1) * 2.2)
      ...
      .shadow(color: tint.opacity(isSelected ? 0.20 : 0.11), radius: isSelected ? 11 : 8, x: 0, y: 4)
      .shadow(color: Color.black.opacity(0.08), radius: 5, x: 0, y: 2)
  }

  private func stackRotation(_ index: Int) -> Double {
    [-3.2, 2.8, -2.2, 1.7][index % 4]
  }

  private func stackOffsetX(_ index: Int) -> CGFloat {
    [-3.0, 2.0, -1.5, 1.0][index % 4]
  }
}
  ...
  var body: some View {
    Button(action: action) {
      CardStackFrame(
        ...
        badge: quarantineMessages.isEmpty ? nil : String(quarantineMessages.count),
        tint: .orange,
...
private struct InboxStackCard: View {
  ...
  var body: some View {
    CardStackFrame(
      depth: stackDepth,
      badge: stackDepth > 1 ? String(stackDepth) : nil,
...
private struct MessageBubble: View {
  ...
  var body: some View {
    HStack {
      ...
      .shadow(color: messageTint.opacity(message.from_self ? 0.16 : 0.10), radius: 6, x: 0, y: 2)
      .contextMenu { MessageContextMenu(message: message) }
  ...
  private var messageBackground: some View {
    RoundedRectangle(cornerRadius: 8)
      .fill(
        LinearGradient(
          colors: [
            messageTint.opacity(message.from_self ? 0.24 : 0.15),
            Color(nsColor: .controlBackgroundColor).opacity(message.from_self ? 0.84 : 0.94),
            messageTint.opacity(message.from_self ? 0.12 : 0.08)
          ],
          startPoint: .topLeading,
          endPoint: .bottomTrailing
        )
