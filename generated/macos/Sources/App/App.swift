  ...
  var body: some View {
    HStack(spacing: 0) {
      VStack(alignment: .leading, spacing: 0) {
        HeaderView(title: "New Senders", subtitle: "\(session.newSenderThreads.count) senders")
        Divider()
        List(selection: $session.selectedNewSenderID) {
          ForEach(session.newSenderThreads) { thread in
            SidebarThreadRow(thread: thread)
              .tag(thread.id)
              .onTapGesture { session.selectNewSender(thread) }
          }
        }
        .listStyle(.inset)
      }
      ...
      Divider()
      VStack(alignment: .leading, spacing: 0) {
        if let sender = session.selectedNewSender {
          HeaderView(title: sender.displayName, subtitle: "\(session.newSenderMessages.count) quarantined messages")
        } else {
          HeaderView(title: "Messages", subtitle: "No sender selected")
        }
        Divider()
        List(session.newSenderMessages) { message in
      ...
      Divider()
      MessageReaderView(message: selectedMessage, emptyTitle: "No Message Selected")
        .frame(minWidth: 420)
    }
  }
}
...
private struct InboxView: View {
  ...
  var selectedMessage: MessageItem? {
    if let id = session.selectedMessageID, let match = session.snapshot.inbox.first(where: { $0.id == id }) {
      return match
    }
    return session.snapshot.inbox.first
  }
    ...
    HStack(spacing: 0) {
      VStack(alignment: .leading, spacing: 0) {
        ...
        ScrollViewReader { proxy in
          List(session.snapshot.inbox) { message in
            MessageListRow(message: message)
              .id(message.id)
              .onTapGesture { session.selectMessage(message) }
      ...
      Divider()
      MessageReaderView(message: selectedMessage, emptyTitle: "Inbox Is Clear")
    }
  }
}
